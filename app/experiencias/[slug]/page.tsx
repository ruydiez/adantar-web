...
export default async function ExperiencePage({
  ...
  const imageUrl = exp.mainImage
    ? urlForImage(exp.mainImage).width(1600).height(900).url()
    : null

  const meta = [
    exp.duration    && { label: 'Duración',   value: exp.duration },
    exp.capacity    && { label: 'Capacidad',  value: exp.capacity },
    exp.destination && { label: 'Destino',    value: exp.destination },
  ...
  return (
    <div className="pt-16 min-h-screen">
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-12 py-16">
        {/* Header */}
        <p className="text-[10px] tracking-[0.2em] uppercase text-arena mb-5">
          {exp.vertical}
        </p>
        ...
        <div className="mt-16 pt-12 border-t border-white/10">
          ...
          <p className="text-white/45 text-sm mb-8 leading-relaxed max-w-md">
            ...
            Sin precios fijos. Habla con nosotros.
          </p>
          <Link
            href={`/contacto?experiencia=${encodeURIComponent(exp.title)}`}
        ...
        <div className="mt-12">
          <Link
            href="/experiencias"
            className="text-[11px] tracking-widest uppercase text-white/30 hover:text-white/60 transition-colors"
          >
            ← Volver al catálogo
          </Link>
        </div>
